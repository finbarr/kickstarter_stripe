module KickstarterStripe
  module Util
    ...
    def self.convert_to_stripe_object(resp, api_key)
      ...
      when Hash
        # Try converting to a known object class.  If none available, fall back to generic KickstarterStripeObject
        object_classes.fetch(resp[:object], KickstarterStripeObject).construct_from(resp, api_key)
      else
        resp
      end
    end
